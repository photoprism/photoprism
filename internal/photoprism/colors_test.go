	...
	*/
	expected := map[string]ColorPerception{
		...
		"testdata/cat_black.jpg": {
			Colors:     IndexedColors{0x2, 0x1, 0x1, 0x1, 0x1, 0x1, 0x2, 0x5, 0x2},
			MainColor:  1,
			Luminance:  LightMap{0x8, 0xb, 0x9, 0x4, 0x2, 0x6, 0xd, 0xc, 0x3},
			Saturation: 2,
		},
		"testdata/cat_brown.jpg": {
			Colors:     IndexedColors{0x9, 0x5, 0x1, 0x2, 0x2, 0x1, 0x1, 0x6, 0x2},
			MainColor:  1,
...
func TestMediaFile_Colors(t *testing.T) {
	...
	t.Run("iphone/IMG_6788.JPG", func(t *testing.T) {
		if mediaFile, err := NewMediaFile(conf.ImportPath() + "/iphone/IMG_6788.JPG"); err == nil {
			...
			assert.Equal(t, IndexedColors{0x6, 0x6, 0x9, 0x1, 0x1, 0x2, 0x1, 0x1, 0x6}, p.Colors)
		} else {
			t.Error(err)
		}
	})
