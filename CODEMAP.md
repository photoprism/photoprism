...
- Signed download tokens: `pkg/authn/authtoken` mints/verifies HMAC-SHA256, session-bound, expiring `?t=` tokens so header-less download endpoints can resolve and scope the requester (docs: `pkg/authn/authtoken/README.md`). Sign/verify live in `internal/auth/tokens`: a generic `Signer` (key + signature path) with one instance per token kind — the `Download` signer plus thin `SignDownload`/`VerifyDownload` wrappers today, previews next — configured by `Config.Propagate` (lifetime in `ttl.DownloadToken`); a Propagate-configured leaf like `thumb`/`dl`/`ttl`, so it imports neither `config` nor `get`. Config keeps the key: `Config.TokenSigningKey` (the shared `config/keys/signing.key` secret — one key signs every token kind, regenerated if missing rather than backed up, and always kept in memory so signing never breaks when the disk is read-only) and `DownloadTokenMaxAge` (the `download-token-maxage` option). Download tokens are **stateless** (no per-session/user storage): `tokens.DownloadToken(sessionID)` delivers the value (public placeholder / configured static / signed / coarse) and `tokens.IsCoarseDownload` / `VerifyDownload` validate it; `Config.Propagate` sets `tokens.PublicMode`/`DownloadStatic`/`CoarseDownload` (the single coarse instance token from `Config.DownloadToken`). Request-side resolution in `internal/api/auth_tokens.go`: `DownloadSession` resolves a signed `?t=` token to its session, public mode to the public session, else nil — and, before the token, accepts a **Portal cluster JWT in a request header** (`authAnyJWT` requiring `acl.AccessAll` on files, so only a trusted full-access principal qualifies; a transient JWT session can't back a `?t=` token, and only JWTs — not arbitrary bearer/Basic-auth headers — take this path, which otherwise falls through to `?t=`). `AuthDownload(c) (sess, valid)` is the merged gate the endpoints use (session-or-coarse authorization + the resolved session in one call, auditing a denial centrally like `AuthAny`); `InvalidDownloadToken` is a thin wrapper. Scoped consumers: `DownloadAlbum` (`internal/api/download_album.go`), `GetDownload` (`internal/api/download.go`), `GetPhotoDownload` (`internal/api/photos.go`), `ZipDownload` (`internal/api/zip.go`).
