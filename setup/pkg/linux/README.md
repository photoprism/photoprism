...
The libheif installer puts its binaries under `/usr/local` alongside the distribution packages rather than replacing them, so nothing is removed and the loader prefers the newer build. It declines to install an older version over a newer one already in that prefix, since the result would be every program on the system quietly resolving to the older build without any package manager reporting it; pass `--downgrade` when that is what you want. Our Docker images take a different path, where a single libheif is installed through apt — `--deb` selects it, and it replaces the distribution packages and anything depending on them. That path needs `apt`, so it is unavailable on Fedora and openSUSE — where no build is published today either, and the installer says so and exits.
