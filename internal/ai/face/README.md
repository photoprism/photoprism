...
- **Scores are detector confidence in percent, 0-100, and neither bar is shared.** `FACE_SCORE` is the base minimum, applied in `face.Detect` so a detection below it never becomes a marker; unset, each detector's own `MinScore` decides. `FACE_CLUSTER_SCORE` is the higher bar for automatic clustering, and `face.ClusterScore` looks it up from **the detector that produced each marker** (`Detector.ClusterMinScore`: YuNet 70, SCRFD 60), not from the one in force. A library holds markers from more than one detector and nothing recomputes a score, so a shared bar would exclude old markers for a calibration they were never scored against - permanently. Rows written before `markers.detect_model` existed keep `ClusterScoreThresholdDefault`, so an upgrade strands nothing.
  ...
  **Each detector also carries its own cutoff** (`Detector.MinScore`), because they do not score alike: SCRFD emits one calibrated sigmoid and sits at 0.50, while YuNet scores as `sqrt(cls x obj)` and needs 0.65. Measured over 173 group photographs and the detector test images, 0.65 is where YuNet stops accepting flowers and statues as faces while still finding more in a crowd than SCRFD does - at 0.70 it loses a real face and its advantage on groups with it. A cutoff copied from another detector is not calibration, which `TestDetectorMinScore` states.
...
| `FACE_SCORE`            | *(detector)*                           | Minimum detection quality, applied in `face.Detect`. Unset, each detector's own calibrated cutoff decides; a value set here applies on top of it, in either direction.                   |
