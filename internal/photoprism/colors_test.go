	...
	*/
	expected := map[string]ColorPerception{
		"testdata/sharks_blue.jpg": {
			...
			MainColor:  6,
			Luminance:  LightMap{0x9, 0x7, 0x5, 0x6, 0x5, 0x6, 0x5, 0x5, 0x5},
			Saturation: 15,
		},
		"testdata/cat_black.jpg": {
			Colors:     IndexedColors{0x2, 0x5, 0x1, 0x1, 0x1, 0x1, 0x2, 0x2, 0x2},
			MainColor:  1,
			Luminance:  LightMap{0x8, 0xb, 0x9, 0x4, 0x2, 0x6, 0xd, 0xc, 0x3},
			Saturation: 2,
		},
		"testdata/cat_brown.jpg": {
			Colors:     IndexedColors{0x1, 0x5, 0x5, 0x2, 0x2, 0x1, 0x1, 0x6, 0x2},
			MainColor:  5,
			Luminance:  LightMap{0x3, 0x4, 0xa, 0x4, 0x7, 0x2, 0x1, 0x5, 0x6},
			Saturation: 4,
		},
		"testdata/cat_yellow_grey.jpg": {
			Colors:     IndexedColors{0x2, 0x1, 0x2, 0x1, 0x1, 0x5, 0x5, 0x1, 0x5},
			MainColor:  5,
			Luminance:  LightMap{0x8, 0x5, 0xa, 0x5, 0x2, 0x5, 0x7, 0x1, 0x6},
			Saturation: 4,
		},
	}
...
func TestMediaFile_Colors(t *testing.T) {
	...
	t.Run("dog.jpg", func(t *testing.T) {
		if mediaFile, err := NewMediaFile(conf.ImportPath() + "/dog.jpg"); err == nil {
			...
			assert.Nil(t, err)
			assert.Equal(t, 5, p.Saturation.Int())
			assert.IsType(t, IndexedColors{}, p.Colors)
			assert.Equal(t, "brown", p.MainColor.Name())
			assert.Equal(t, IndexedColors{0x1, 0x3, 0x1, 0x2, 0xe, 0x1, 0x2, 0x2, 0x0}, p.Colors)
			assert.Equal(t, LightMap{0x4, 0xf, 0x7, 0xc, 0x4, 0x2, 0x4, 0x3, 0x0}, p.Luminance)
		} else {
			t.Error(err)
		}
	})
	...
	t.Run("iphone/IMG_6788.JPG", func(t *testing.T) {
		if mediaFile, err := NewMediaFile(conf.ImportPath() + "/iphone/IMG_6788.JPG"); err == nil {
			...
			assert.Nil(t, err)
			assert.Equal(t, 4, p.Saturation.Int())
			assert.IsType(t, IndexedColors{}, p.Colors)
			assert.Equal(t, "blue", p.MainColor.Name())
			assert.Equal(t, IndexedColors{0x6, 0x6, 0x5, 0x1, 0x1, 0x2, 0x1, 0xf, 0x6}, p.Colors)
		} else {
			t.Error(err)
		}
	})
	...
	t.Run("raw/20140717_154212_1EC48F8489.jpg", func(t *testing.T) {
		if mediaFile, err := NewMediaFile(conf.ImportPath() + "/raw/20140717_154212_1EC48F8489.jpg"); err == nil {
			...
			assert.Equal(t, IndexedColors{0x3, 0x3, 0x6, 0x1, 0x2, 0x2, 0x5, 0x5, 0x1}, p.Colors)
		} else {
			t.Error(err)
		}
	})
