...
type XmpDocument struct {
	...
	RDF     struct {
		...
		Description struct {
			...
			RatingAttr      string `xml:"http://ns.adobe.com/xap/1.0/ Rating,attr"`        // 4
			RatingPlainAttr string `xml:"Rating,attr"`                                     // 4
			UserRating      string `xml:"UserRating"`                                      // 4
			UserRatingAttr  string `xml:"http://ns.adobe.com/xap/1.0/ UserRating,attr"`    // 4
			UserRatingPlain string `xml:"UserRating,attr"`                                 // 4
			RatingPercent   string `xml:"RatingPercent"`                                   // 50
			RatingPctAttr   string `xml:"http://ns.adobe.com/xap/1.0/ RatingPercent,attr"` // 50
			RatingPctPlain  string `xml:"RatingPercent,attr"`                              // 50
...
// Rating returns a normalized 0-5 star rating from the XMP document.
func (doc *XmpDocument) Rating() (rating int, ok bool) {
	if rating, ok = ParseRating(doc.RDF.Description.RatingAttr); ok {
		return rating, true
	}

	if rating, ok = ParseRating(doc.RDF.Description.RatingPlainAttr); ok {
		return rating, true
	}

	if rating, ok = ParseRating(doc.RDF.Description.Rating); ok {
		return rating, true
	}

	if rating, ok = ParseRating(doc.RDF.Description.UserRatingAttr); ok {
		return rating, true
	}

	if rating, ok = ParseRating(doc.RDF.Description.UserRatingPlain); ok {
		return rating, true
	}

	if rating, ok = ParseRating(doc.RDF.Description.UserRating); ok {
		return rating, true
	}

	if rating, ok = ParseRatingPercent(doc.RDF.Description.RatingPctAttr); ok {
		return rating, true
	}

	if rating, ok = ParseRatingPercent(doc.RDF.Description.RatingPctPlain); ok {
		return rating, true
	}

	return ParseRatingPercent(doc.RDF.Description.RatingPercent)
}
