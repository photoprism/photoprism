  ...
  <div>
    <v-container v-if="selection.length > 0" fluid class="pa-0">
      ...
      >
        ...
        <v-btn key="download" v-if="$config.feature('download')" theme="dark" size="small" :title="$gettext('Download')" color="download" class="action-download rounded-circle" :disabled="selection.length === 0" @click.stop="download()">
          <v-icon>mdi-download</v-icon>
        </v-btn>

        <v-btn v-if="$config.feature('albums')" key="bookmark" theme="dark" size="small" :title="$gettext('Add to album')" color="album" :disabled="selection.length === 0" class="action-album rounded-circle" @click.stop="dialog.album = true">
