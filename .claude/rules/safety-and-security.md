...
- Where this lives: `pkg/fs/copy_move.go` (`fs.Copy` / `fs.Move`) and `internal/service/webdav/client.go` (`Client.Download`) enforce it. `internal/photoprism/mediafile.go` (`MediaFile.Copy/Move`) is the third site and does not yet.
