...
- **Verify `scripts/dist/convert/policy.xml` rules by probing ImageMagick, never by reading them.** Patterns match the names ImageMagick registers (`magick -list format` / `-list delegate`), not format names, and an unmatched pattern is silently ignored while still printing in `magick -list policy` — so an inert rule looks exactly like an enforced one. Which coders must stay enabled, how the cache limits track the resolution options, and which settings break conversion are documented in the related specs.
