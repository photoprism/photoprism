...
- **Scores are detector confidence in percent, 0-100.** `ScoreThreshold` (`FACE_SCORE`, default 65) is the base minimum and `ClusterScoreThreshold` (`FACE_CLUSTER_SCORE`, default 70) the higher bar for automatic clustering. Both live in `internal/ai/face/config.go`, and `face.Detect` applies the first, so a detection below it never becomes a marker rather than being filtered out of one place and not another.
...
| `FACE_SCORE`            | `65`                                   | Minimum detection quality, applied in `face.Detect` so a low-scoring detection never becomes a marker. The detectors carry their own calibrated cutoffs, which are stricter, so raising this is what has an effect. |
