...
func Register(fileUuid, fileName string) error {
	if !rnd.IsUUID(fileUuid) {
		event.AuditWarn([]string{"api", "download", "create temporary token for %s", status.Failed}, clean.Log(fileName))
	...
	if fileName = fs.Abs(fileName); !fs.FileExists(fileName) {
		...
		event.AuditErr([]string{"api", "download", "create temporary token for %s", status.Denied}, clean.Log(fileName))
		return errors.New("forbidden file path")
	}

	event.AuditInfo([]string{"api", "download", "create temporary token for %s", status.Succeeded}, clean.Log(fileName))
