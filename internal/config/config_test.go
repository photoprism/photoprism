...
func TestConfig_SaveOptionsUpdate(t *testing.T) {
	newTestOptionsConfig := func(t *testing.T) *Config {
		t.Helper()
	...
	t.Run("RemovesKeys", func(t *testing.T) {
		c := newTestOptionsConfig(t)
		require.NoError(t, os.WriteFile(c.OptionsYaml(), []byte("FaceEngine: none\nFaceModel: sface\n"), fs.ModeConfigFile))

		changed, err := c.SaveOptionsUpdate(Values{"FaceDetector": "none"}, []string{"FaceEngine"})
		...
		assert.Equal(t, "none", values["FaceDetector"])
		assert.Equal(t, "sface", values["FaceModel"])
	})
	t.Run("NothingToDo", func(t *testing.T) {
		c := newTestOptionsConfig(t)
		require.NoError(t, os.WriteFile(c.OptionsYaml(), []byte("FaceModel: sface\n"), fs.ModeConfigFile))

		changed, err := c.SaveOptionsUpdate(nil, []string{"FaceEngine"})
		require.NoError(t, err)
		assert.False(t, changed)
	})
	t.Run("NoPatchAndNoRemoval", func(t *testing.T) {
		c := newTestOptionsConfig(t)

		changed, err := c.SaveOptionsUpdate(nil, nil)
		require.NoError(t, err)
		assert.False(t, changed)
	})
}
