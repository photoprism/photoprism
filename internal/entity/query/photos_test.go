...
func TestPhotos_CountPhotos(t *testing.T) {
	count, err := CountPhotos()
	assert.NoError(t, err)
	assert.Greater(t, count, 57)
}

// By is the type of a "less" function that defines the ordering of its Photo arguments.
type By func(p1, p2 *entity.Photo) bool

// Sort is a method on the function type, By, that sorts the argument slice according to the function.
func (by By) Sort(photos []*entity.Photo) {
	ps := &photoSorter{
		photos: photos,
...
type photoSorter struct {
	photos []*entity.Photo
	by     func(p1, p2 *entity.Photo) bool // Closure used in the Less method.
}
...
func TestPhotos_UnscopedSearch(t *testing.T) {
	t.Run("Ok", func(t *testing.T) {
		...
		if res := UnscopedSearchPhotos(&photos, "photo_uid in (?, ?, ?)", entity.PhotoFixtures.Get("Photo07").PhotoUID, entity.PhotoFixtures.Get("Photo08").PhotoUID, entity.PhotoFixtures.Get("Photo18").PhotoUID); res.Error != nil {
			assert.Nil(t, res.Error)
			t.FailNow()
		}
		photo1 := entity.PhotoFixtures.Get("Photo08")
		photo2 := entity.PhotoFixtures.Get("Photo07")
		photo3 := entity.PhotoFixtures.Get("Photo18")

		expectedPhotos := entity.Photos{&photo2, &photo1, &photo3}

		r := photos.Photos()

		assert.Equal(t, 3, len(r))

		uuid := func(p1, p2 *entity.Photo) bool {
	...
	t.Run("Error", func(t *testing.T) {
		...
		res := &gorm.DB{}
		log.Info("Expect unknown column Error or SQLSTATE on photo_uids from UnscopedSearchPhotos")
...
func TestPhotos_ScopedSearch(t *testing.T) {
	t.Run("Ok", func(t *testing.T) {
		...
		if res := ScopedSearchPhotos(&photos, "photo_uid in (?, ?, ?)", entity.PhotoFixtures.Get("Photo07").PhotoUID, entity.PhotoFixtures.Get("Photo08").PhotoUID, entity.PhotoFixtures.Get("Photo18").PhotoUID); res.Error != nil {
			assert.Nil(t, res.Error)
			t.FailNow()
		}
		photo1 := entity.PhotoFixtures.Get("Photo08")
		...
		expectedPhotos := entity.Photos{&photo2, &photo1}
	...
	t.Run("Error", func(t *testing.T) {
		...
		res := &gorm.DB{}
		log.Info("Expect unknown column Error or SQLSTATE on photo_uids from ScopedSearchPhotos")
