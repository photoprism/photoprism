...
- The OIDC callback template clears only **this app's namespaced** session data keys from `localStorage` and `sessionStorage`. It MUST NOT touch the un-namespaced legacy/global keys: on a shared-domain cluster (`/i/<tenant>/`) every instance and the Portal share one origin, so a global clear during one instance's OIDC completion would wipe a sibling instance's or the Portal's session. The namespaced `session` storage-preference flag is preserved so ephemeral `sessionStorage` flows stay logged in after redirect.
- The OIDC callback reads only the namespaced `localStorage["pp:<namespace>:session"] === "true"` preference when choosing the preferred write target; legacy un-namespaced keys are migrated lazily on read by `Session`/`NamespacedStorage`, never cleared cross-namespace by the callback.
