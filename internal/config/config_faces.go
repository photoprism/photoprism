...
func (c *Config) FaceModel() string {
	...
	if name := c.libraryFaceModel(); name == "" {
		...
		// Anything indexed from now on lands in a second vector space that cannot be
		// compared with what the library already holds, so this needs to be visible.
		log.Warnf("config: library was indexed with face model %s, which is not installed", clean.Log(name))
		resolved = installedFaceModel(modelsPath)
	}
