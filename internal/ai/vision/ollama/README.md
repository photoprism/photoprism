...
The table below reports median single-image latency over a fixed 16-image benchmark. All six models returned well-formed JSON for every request, so the differences are in verbosity, speed, and how often a model guesses rather than in reliability.

| Model (Cloud Tag)      | Latency (caption / labels) | Labels/Image | Strengths                                                                         | Notes                                                                                     |
|:-----------------------|:---------------------------|-------------:|:----------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------|
| `kimi-k3:cloud`        | ~1 s / ~4 s                |          9.2 | Best overall subject accuracy; the most concise captions (~16 words)              | Occasionally emits phrases (`ferris wheel`); see the note on single-word nouns above      |
| `kimi-k2.7-code:cloud` | ~1 s / ~2 s                |          9.0 | Matches `kimi-k3` on accuracy at roughly half the label latency; no phrase labels | Label-set size varies more per image (5-18)                                               |
| `minimax-m3:cloud`     | ~3 s / ~5 s                |         12.4 | Largest label sets, multilingual, never breaks the single-word contract           | **Current default.** Slowest of the fast group; extra labels trend abstract (`structure`) |
| `kimi-k2.6:cloud`      | ~2 s / ~4 s                |          8.6 | Same family as `kimi-k3`, slightly more verbose captions                          | Superseded by `kimi-k3` on both speed and accuracy                                        |
| `gemma4:31b-cloud`     | ~1 s / ~2 s                |          7.0 | Fastest of the group; consistent Gemma-family JSON                                | Most conservative labeler; misreads some subjects (skier as snowboarder, omits `dome`)    |
| `qwen3.5:397b-cloud`   | ~4 s / ~6 s                |          7.4 | Strong multilingual captions and labels                                           | Slowest overall; returns Title-Cased phrases more often than the others                   |
