...
// libraryFaceModel returns the embedding model that produced most of the face vectors
// stored in the library, or an empty name when it holds none and when the database is
// not connected yet. Vectors predating the provenance column were generated by FaceNet.
func (c *Config) libraryFaceModel() face.ModelName {
	...
	// The schema is migrated after the configuration is propagated, so on the first start
	// after an upgrade the provenance column does not exist yet. A library that predates
	// it cannot hold anything but FaceNet vectors, which is what its face markers prove.
...
// empty name when none were counted. The query only counts markers that hold a vector,
// so a blank model name means the vector predates the provenance column and is FaceNet.
