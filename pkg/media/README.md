...
Insta360 cameras record 360° originals in two proprietary extensions: `.insp` is a JPEG that holds both fisheye images side by side in a 2:1 frame (single-lens modes write other aspect ratios, which are not dewarped), and `.insv` is an MP4-style container with dual-fisheye video, either side by side in one stream or, on the X4 and later, as one stream per lens, plus a gyro/metadata track. Neither is stitched, so both need a dewarp to equirectangular before a sphere viewer can show them.

One capture is not always one file. Higher-resolution models write each lens to its own original (`VID_<date>_<time>_00_<seq>.insv` and `..._10_...`) alongside an optional low-resolution proxy (`LRV_..._11_...`). `ParseInsta360VideoName` recognizes this layout so the indexer can treat the set as a single panorama instead of three unrelated videos. It matches names against `fs.Insta360VideoPattern`, which `fs.StackPrefix` also uses to stack the files of one capture under the name of the `_00` file. Models that store both lenses in one `.insv` write their proxy as `LRV_<date>_<time>_01_<seq>.lrv` (file type `lrv`), which `fs.StackPrefix` maps to the `_00` name through `fs.Insta360ProxyPattern`.
