...
- Stacking names: `stack.go` (`StackPrefix`, `StackGroup`, `KeepStacked`, `Insta360VideoPattern`).
...
- Stacking vs. file names: `StackPrefix` returns the name under which a file is stacked with a photo. It maps the lens, proxy, and derived sidecar files of an Insta360 separate-lens capture (`VID|LRV_…insv`) to the name of the `_00` file, whether or not sequences are stripped; for any other name it equals `BasePrefix`. Use `BasePrefix`, `AbsPrefix`, and `RelPrefix` wherever a file's own name is needed, e.g. to find or rename its sidecars.
