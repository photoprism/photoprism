...
// by name. Markers are what a reset regenerates, so their counts show how much of a
...
// embedding model other than the one currently configured. Clusters without a recorded
// model are not counted, because rows created before provenance was tracked have none.
func FacesFromOtherModels() (count int, err error) {
	...
	err = Db().
		Table(entity.Face{}.TableName()).
		Where("embed_model <> '' AND embed_model <> ?", current).
		Count(&count).Error
