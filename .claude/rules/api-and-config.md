...
- Flag defaults: `DocDefault` only affects the generated reference, **not** `--help`. For a numeric option whose getter treats `0` as "use the default", also set `Value:` to that default — otherwise `photoprism --help` prints a misleading `(default: 0)`. See `session-maxage` / `session-timeout` / `session-cache` for the pattern.
