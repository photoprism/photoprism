...
This package holds one description of an ONNX model's parameters, shared by the subsystems that run one. The field list is the description of a graph plus its preprocessing contract, and that does not vary by task, so labels, NSFW detection, face detection, and face embeddings use the same structure rather than separate runtime registries.
...
| NSFW (`internal/ai/nsfw`)             | Refuse to load   | A mismatched preprocessing contract silently changes the safety decision                                         |
...
- `internal/ai/nsfw` — `Models` adds output reduction, class indices, and calibrated thresholds, while the shared description controls session validation and preprocessing.
