...
- **AppArmor profiles cannot be shipped inside an image.** Policy is a host-kernel object; a profile baked into an image is inert unless the operator loads it and passes `security_opt: apparmor=<name>`. So "ship a purpose-written profile" is never a fix for a confined delegate — install the binary at a path the profile does not attach to, or stop depending on it.
- **Prove every rule in `scripts/dist/convert/policy.xml` with a probe, never by reading it.** The shared ImageMagick policy (installed at `/etc/ImageMagick-7/policy.xml` by the current `-slim` bases, and at the `ImageMagick-6` path by `plus/docker/legacy`) matches the names ImageMagick *registers*, not format names: the URL delegates are `http:decode` / `https:decode`, so a `pattern="HTTP"` delegate rule matches nothing. An unmatched pattern is silently ignored yet still prints in `magick -list policy`, so an inert rule is indistinguishable from an enforced one by inspection. Discover names with `magick -list format` (coder + module) and `magick -list delegate`, then confirm the behavior itself. `MVG`/`MSVG` must stay enabled (SVG renders through them) and `PDF`/`PS`/`EPS`/`AI` stay enabled for document and vector previews. Size the pixel-cache limits against `PHOTOPRISM_RESOLUTION_LIMIT` (default 150 MP, max 900) and the 30000 px `PHOTOPRISM_JPEG_SIZE` ceiling — undersized limits reject valid media with `cache resources exhausted` or `width or height exceeds limit`. Do not enable `list-length`: it hard-errors instead of truncating, breaking animated WebP/GIF conversion.
